...
SNAKE_BOSS_START_SEGMENTS = 5  # Starting number of body segments (not including head, adjustable)
...
SNAKE_BOSS_HEAD_HEALTH_BASE = 3  # Base health for head final phase (adjustable)
...
class LevelUpScreen:
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            # Check for remapped up/down buttons
            # Check if it's an up or down button press
            elif is_button_pressed(event, self.key_bindings, 'player1_up_button') or is_button_pressed(event, self.key_bindings, 'player2_up_button'):
                if not self.is_coop:
                    # Single player controller
                    if self.sound_manager:
                        self.sound_manager.play_sound('menu_change')
                    ...
                    # Co-op - determine which player by controller ID
                    player1_controller_id = self.get_player_controller_id(0)
                    player2_controller_id = self.get_player_controller_id(1)
                    ...
                    if player1_controller_id is not None and event.joy == player1_controller_id and not self.player1_confirmed:
                        ...
                        p1_options = self.get_options_for_player(0)
                        self.player1_selection = (self.player1_selection - 1) % len(p1_options)
                    elif player2_controller_id is not None and event.joy == player2_controller_id and not self.player2_confirmed:
                        if self.sound_manager:
                            self.sound_manager.play_sound('menu_change')
                        p2_options = self.get_options_for_player(1)
                        self.player2_selection = (self.player2_selection - 1) % len(p2_options)
            elif is_button_pressed(event, self.key_bindings, 'player1_down_button') or is_button_pressed(event, self.key_bindings, 'player2_down_button'):
                if not self.is_coop:
                    # Single player controller
                    if self.sound_manager:
                        self.sound_manager.play_sound('menu_change')
...
class AchievementScreen:
    ...
    def _scroll_by(self, direction):
        self.scroll_offset = max(
            0,
            min(self.max_scroll, self.scroll_offset + (direction * self.scroll_speed))
        )

    def _begin_scroll_hold(self, direction):
        now = pygame.time.get_ticks()
        self.scroll_hold_direction = direction
        self.scroll_hold_start = now
        self.scroll_hold_last = now
        self._scroll_by(direction)

    def _end_scroll_hold(self, direction):
        if self.scroll_hold_direction == direction:
            self.scroll_hold_direction = 0

    def _update_scroll_hold(self):
        if self.scroll_hold_direction == 0:
            return

        keys = pygame.key.get_pressed()
        if self.scroll_hold_direction < 0:
            held = keys[pygame.K_UP] or keys[pygame.K_w]
        else:
        ...
        now = pygame.time.get_ticks()
        elapsed = now - self.scroll_hold_start
    ...
    def draw(self):
        ...
        visible_height = SCREEN_HEIGHT - y_start - 100
        ...
        if self.max_scroll > 0:
            ...
            track_x = SCREEN_WIDTH - 70
            track_y = y_start
            track_width = 12
            pygame.draw.rect(self.screen, GRAY, (track_x, track_y, track_width, track_height), border_radius=6)

            thumb_height = max(40, int(track_height * (visible_height / total_height)))
            thumb_range = track_height - thumb_height
            thumb_offset = int((self.scroll_offset / self.max_scroll) * thumb_range)
            pygame.draw.rect(
                self.screen,
                WHITE,
        ...
        if self.max_scroll > 0:
            instructions.insert(0, "Hold UP/DOWN to scroll")
...
class RogueTerminalBoss:
    ...
    def __init__(self, encounter):
        ...
        self.taunt_messages = [
            ...
            "YOU ARE A BUG TO BE SQUASHED",
            "MY PROCESSES ARE INFINITE",
            "SURRENDER YOUR SHIP",
        ]
        self.current_taunt = ""
        self.taunt_until = 0
    ...
    def update(self, players=None, sound_manager=None):
        ...
        if now - self.last_phase_change >= self.phase_duration:
            ...
            self.glitch_until = now + 400
            self.current_taunt = random.choice(self.taunt_messages)
            self.taunt_until = now + 2000
    ...
    def shoot(self, players, sound_manager=None):
        ...
        cooldown_map = {
            "BOOT_SEQ": ROGUE_TERMINAL_BOSS_BOOT_SHOT_COOLDOWN,
            "FIREWALL": ROGUE_TERMINAL_BOSS_SCAN_SHOT_COOLDOWN,
    ...
    def create_final_explosion(self):
        ...
        for _ in range(90):
            particles.append({
                ...
                'y': self.y + random.randint(-50, self.height + 50),
                'vel_x': random.uniform(-8, 8),
                'vel_y': random.uniform(-9, 3),
                'color': random.choice([(0, 255, 80), (255, 60, 40), (100, 180, 255), (255, 255, 255), (255, 200, 0)]),
                'size': random.randint(3, 11),
                'life': random.randint(1000, 1800),
                'gravity': random.uniform(0.08, 0.25)
            })
        return particles
    ...
    def draw(self, screen):
        ...
        for row, col, char, x, y, _ in self._symbol_cells():
            ...
            if is_glitching and random.random() < 0.3:
                symbol_color = random.choice([WHITE, CYAN, RED, YELLOW])
            glyph = self.font.render(char, True, symbol_color)
            screen.blit(glyph, (x, y))

        for sy in range(int(self.y), int(self.y + self.height), 4):
            scanline = pygame.Surface((self.width, 1), pygame.SRCALPHA)
            scanline.fill((0, 0, 0, 25))
            screen.blit(scanline, (int(self.x), sy))

        if now < self.taunt_until and self.current_taunt:
            taunt_font = pygame.font.Font("assets/fonts/PressStart2P-Regular.ttf", 11)
            blink = (now // 300) % 2 == 0
            taunt_color = phase_color if blink else (255, 255, 255)
            taunt_surf = taunt_font.render(f"> {self.current_taunt}", True, taunt_color)
            tx = int(self.x + self.width // 2 - taunt_surf.get_width() // 2)
            screen.blit(taunt_surf, (tx, int(self.y + self.height + 8)))

        ratio = self.health / self.max_health
        bar_y = int(self.y - 26)
        bar_bg = (60, 0, 0)
        bar_fill = tuple(min(255, c) for c in phase_color)
        pygame.draw.rect(screen, bar_bg, (int(self.x), bar_y, self.width, 14))
        pygame.draw.rect(screen, bar_fill, (int(self.x), bar_y, int(self.width * ratio), 14))
        pygame.draw.rect(screen, phase_color, (int(self.x), bar_y, self.width, 14), 2)
        form = self._current_form()
        status_msgs = {
            "BOOT_SEQ": "NEXUS-9 [BOOT_SEQ] TARGETING...",
            "FIREWALL": "NEXUS-9 [FIREWALL] INTRUDER DETECTED",
            "MELTDOWN": "NEXUS-9 [MELTDOWN] CORE CRITICAL",
        }
        label = status_msgs.get(form["name"], f"NEXUS-9 [{form['name']}]")
        label += f"  {self.health}/{self.max_health}"
        text = pygame.font.Font("assets/fonts/PressStart2P-Regular.ttf", 14).render(label, True, form["color"])
